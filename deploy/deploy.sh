...
destAbsPath=/home/xzuser/application/$projectName
...
sourFile=$WORKSPACE/$projectName/target/$projectName*.jar
...
function run(){
   ...
   nohup nice java -jar $destFile --server.port=$serverPort --spring.cloud.config.profile=$env $> $dstLogFile 2>&1 &

   echo "COMMAND: nohup nice java -jar $destFile --server.port=$serverPort $properties --spring.cloud.config.profile=$env $> $dstLogFile 2>&1 &"
